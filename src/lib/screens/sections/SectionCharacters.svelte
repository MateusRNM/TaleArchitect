<script lang="ts">
    ...
    function openEdit(char: any) {
        ...
        formName = char.name;
        formDesc = char.description;
        formImage = char.image;
        view = 'form';
    }
    ...
    function saveCharacter() {
        ...
        if (selectedCharId) {
            projectStore.current?.updateCharacter(selectedCharId, {
                name: formName,
                description: formDesc,
                image: formImage
            });
        } else {
            projectStore.current?.addCharacter(formName, formDesc, formImage);
        }
        view = 'list';
    }
    ...
    async function deleteCharacter() {
        ...
        if (selectedCharId && confirmed) {
            projectStore.current?.removeCharacter(selectedCharId);
            view = 'list';
        }
    }
</script>
    ...
    {:else}
        <div class="max-w-4xl mx-auto w-full h-full flex flex-col p-6 animate-in slide-in-from-right-8 duration-300">
            ...
            <div class="flex items-center justify-between mb-8">
                <button onclick={() => view = 'list'} class="flex items-center gap-2 text-text-muted hover:text-primary transition-colors cursor-pointer">
                    <ArrowLeft size={20} />
                    Voltar
                </button>
                ...
                <div class="flex gap-3">
                    {#if selectedCharId}
                        <button onclick={deleteCharacter} class="p-2 text-red-500 hover:bg-red-100 rounded-lg transition-colors cursor-pointer" title="Deletar">
                            <Trash2 size={20} />
                        </button>
                    {/if}
                    <button onclick={saveCharacter} class="flex items-center gap-2 bg-primary text-background px-6 py-2 rounded-lg font-bold shadow-md hover:brightness-110 cursor-pointer">
                        <Save size={18} />
                        Salvar
                    </button>
                </div>
            ...
            <div class="flex flex-col md:flex-row gap-8 flex-1 overflow-y-auto scrollbar-hide p-1">
                ...
                <div class="w-full md:w-1/3 flex flex-col gap-4">
                    <div class="aspect-3/4 rounded-xl border-2 border-dashed border-text-muted/30 bg-surface/50 overflow-hidden relative group">
                        {#if formImage}
                            <img src={convertFileSrc(formImage)} alt="Preview" class="w-full h-full object-cover" />
                        {:else}
                    ...
                    <button 
                        onclick={() => {
                            ...
                            if(selectedCharId != null) {
                                projectStore.current?.updateCharacter(selectedCharId, { image: undefined });
                            }
                        }}
